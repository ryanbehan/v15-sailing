...
- CMS/Git Gateway: **deferred to backlog** (user decision — not ready for CMS features). config.yml already `backend: name: git-gateway`; /admin/ loads but cannot save until Netlify Identity + Git Gateway are enabled in the dashboard. Activation steps recorded in `memory-bank/taskPlan.md` → Backlog.
